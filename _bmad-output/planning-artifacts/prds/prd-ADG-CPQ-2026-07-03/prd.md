...
- Số sự cố "logic BOM sai làm giá sai hàng loạt" = 0 (bảo vệ bằng nghiệm thu workbook + phiên bản hoá, FR-090/091).
...
2. Cấu hình sản phẩm data-driven: thông số, linh kiện + **lọc tương thích** [D2], phụ kiện, **BOM tính qua workbook Excel của công ty** (Excel-as-engine, hợp đồng INPUT/OUTPUT — [D14]), giá (gross 2 tầng, min–max [D3]).
...
5. Quản lý workbook BOM có phiên bản + **nghiệm thu bằng bộ ca mẫu** [D14].
...
Công ty ra dòng cửa cuốn mới. Chị Mai (admin) tạo sản phẩm, khai bộ thông số (cao, rộng, màu — màu là dropdown), gán các loại linh kiện kèm công thức auto-fill và điều kiện tương thích ("bộ tời X: diện tích ≤ 25m²"), rồi **upload workbook BOM** — chính file Excel tính vật tư của công ty, chuẩn hoá sheet INPUT/OUTPUT [D14] — và đặt gross + range min–max. Chị chạy **nghiệm thu bộ ca mẫu**: hệ thống bơm thông số vào file, so kết quả với đáp án của người làm giá, khớp 100% mới cho dùng. Logic tính vẫn nằm trong file Excel quen thuộc; sai chỗ nào sửa file, upload phiên bản mới. Mọi thay đổi giá/phiên bản đều có vết trong audit log.
...
| FR-021 | **Workbook BOM Excel theo sản phẩm [D14]**: upload có phiên bản + checksum, kiểm tra hợp đồng INPUT/OUTPUT (ô đặt tên khớp thông số; bảng OUTPUT chuẩn; mã vật tư tồn tại), **đúng một version active mỗi sản phẩm** [F4]; logic tính nằm nguyên trong file, hệ thống không tái hiện | UC-A6 |
| FR-022 | Ngôn ngữ công thức trong hệ thống (số học, so sánh, logic, `if(...)`, `round/ceil/floor/min/max`, biến `proj.*`/`comp.*`) **chỉ dùng cho auto-fill thông số linh kiện và điều kiện tương thích SKU** — BOM không dùng, BOM đi qua workbook [D14] | — |
| FR-023 | Lỗi khi tính (workbook trả lỗi, mã vật tư lạ, vật tư thiếu giá, công thức auto-fill lỗi) → dừng tính, báo lỗi rõ cho đại lý và cảnh báo admin; không bao giờ ra giá sai âm thầm | UC-08 |

### F4 — Định giá (Hệ thống + Admin)

| ID | Yêu cầu | UC |
|---|---|---|
| FR-030 | Tính giá **theo từng hạng mục**: cost_material = (số lượng vật tư từ OUTPUT workbook × đơn giá hệ thống `[OPEN — O9]`) + linh kiện + phụ kiện → giá nhập = cost × (1 + gross%); ghi lại phiên bản workbook đã dùng | UC-08 |
...
### F9 — Nghiệm thu workbook BOM (Admin) — trong MVP

| ID | Yêu cầu | UC |
|---|---|---|
| FR-090 | **Nghiệm thu workbook bằng bộ ca mẫu [D14]**: mỗi ca = bộ thông số + kết quả kỳ vọng do người làm giá xác nhận; hệ thống chạy qua engine, so từng dòng; khớp 100% mới cho version active / sản phẩm go-live. File Excel là **nguồn chân lý của logic BOM**; hệ thống bảo đảm file cho kết quả đúng và không bị đổi ngoài luồng (phiên bản + checksum + audit) | UC-A8 |
| FR-091 | Bộ ca mẫu lưu theo sản phẩm, **tự chạy lại khi upload version workbook mới** — chặn active nếu lệch | UC-A8 |
...
| **Workbook BOM chuẩn hoá** từng sản phẩm (file Excel thật của công ty, sheet INPUT/OUTPUT theo mẫu) | FR-021, FR-090 — go-live từng sản phẩm | `[OPEN]` — chưa có file; không chặn build, chặn go-live. Kèm `[O9]`: OUTPUT trả gì |
| **Engine tính file Excel server-side** (không Excel cài đặt, không macro) | FR-021, FR-030 | Chọn ở chặng kiến trúc (AD-17) — có story spike |
...
| **Pilot** `[ASSUMPTION]` | ~15 đại lý, 1–2 dòng sản phẩm, 4–6 tuần | Workbook sản phẩm pilot qua nghiệm thu bộ ca mẫu (FR-090); đo được G2–G4 |
| **Toàn quốc** | >100 đại lý, thêm dòng sản phẩm dần theo nghiệm thu workbook | Pilot đạt: BOM sai ≤ 1%, thời gian báo giá ≤ 15 phút, không sự cố giá |
...
| — | Workbook BOM chuẩn hoá từng sản phẩm | Công ty | **Chặn go-live từng sản phẩm** (không chặn build) |
| O9 | OUTPUT workbook trả gì: chỉ [mã vật tư + số lượng] (khuyến nghị — giá ở hệ thống) hay cả tiền? | Anh DUong xem file thật | Không chặn build — đổi cách tính cost ở FR-030 |
