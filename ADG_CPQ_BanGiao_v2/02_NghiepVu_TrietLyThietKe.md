...
**Công thức là dữ liệu, không phải code [D10].** Toàn bộ logic tính — auto-fill thông số linh kiện, số lượng vật tư BOM, điều kiện tương thích SKU — lưu dưới dạng biểu thức chuỗi do admin quản lý, tham chiếu biến qua `code` ổn định (`proj.<code>`, `comp.<code>`). Ngôn ngữ công thức hỗ trợ **rẽ nhánh điều kiện** `if(...)` và các hàm `round/ceil/floor/min/max` — vì nghiệp vụ thực tế có những quy tắc kiểu *"cao trên 3m thì dùng định mức khác"*. Đặc tả đầy đủ nằm trong Project Note của file DBML. **Quyết định quan trọng [D10]:** toàn bộ logic BOM nằm **trong hệ thống** (không dùng Excel làm engine) vì phiếu BOM sản xuất gửi nhà máy và nhiều logic phức tạp vượt khả năng Excel; bảng tính hiện hành của công ty (khi được chốt) sẽ được **AI-import một lần và nghiệm thu bằng golden tests** (UC-A8), sau đó hệ thống là nguồn chân lý duy nhất.
