...
| D10 | Engine BOM: logic nằm trong hệ thống, **IT/Data Analyst nhập tay** *(tái khẳng định + làm rõ 03/07 — sau khi đã cân nhắc và loại phương án Excel-as-engine INPUT/OUTPUT)* | Chốt: **toàn bộ logic BOM nằm trong hệ thống**, không dùng Excel làm engine. Ngôn ngữ công thức phải gánh **mọi công thức** của Excel giá thật: if/else, **bảng tra `lookup()`** (thay VLOOKUP/ma trận định mức), **biến trung gian `calc.*`** (thay ô-tham-chiếu-ô). Khi bảng tính chính thức được chốt, **IT/Data Analyst (dùng vai admin — không thêm vai mới)** đọc file Excel và nhập công thức + bảng tra vào hệ thống, nghiệm thu bằng golden tests (khớp 100% với Excel gốc mới go-live, tự chạy hồi quy khi sửa công thức); sau đó hệ thống là nguồn chân lý duy nhất — khoá sửa Excel. | UC-A6 (thêm bảng tra + biến trung gian), UC-A8 (nhập tay + golden tests), ERD thêm `lookup_table`, `lookup_entry`, `product_calc_var`, `golden_test_case`. F4 giữ hiệu lực. |
...
| F4 | `bom_template` cho phép nhiều template active cùng lúc cho một sản phẩm; UC-07 mặc nhiên số ít. | Ràng buộc: đúng **một** BOM active mỗi sản phẩm (unique partial index hoặc validate tầng app). |
...
| ~~O6~~ | **ĐÃ ĐÓNG → D10** (02/07, làm rõ 03/07): logic BOM nằm trong hệ thống, IT/Data Analyst nhập tay từ Excel. Còn phụ thuộc: bảng tính chính thức chưa chốt — chờ công ty để nhập công thức + golden tests. | — | — |
...
- Không lưu **lịch sử giá vật tư** ở MVP (giá nằm trong hệ thống — theo D10).
